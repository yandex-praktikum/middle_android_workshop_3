...
@Composable
private fun ScreenContent(state: State, dispatch: (Message) -> Unit) {
    when (state) {
        is State.Loading -> Text("Loading...")
        is State.Content -> Content(state, dispatch)
    }
}

@Composable
private fun Content(state: State.Content, dispatch: (Message) -> Unit) {
    LazyColumn(
        ...
        items(state.items) { item ->
            ItemRow(
                item = item,
                onLikeClicked = { dispatch(Message.ToggleLike(item, !item.isLiked)) })
        }
    }
}